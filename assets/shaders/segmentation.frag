...
uniform int head;
uniform vec3 colours_of_interest[12];
...
vec4 golden_gaussian_3d(vec2 center) {
	vec3 scale = float(Z_SAMPLES) / 2.0 / vec3(textureSize(texture0, 0));
	vec4 color = vec4(0.0);
	float initial = noise(center) * (2.0 * PI);
	for (int i = 0; i<Z_SAMPLES; i++) {
		float theta = initial + HALF_GOLDEN_ANGLE * float(i); //prolly wrong
